...
const adminRequest = vi.hoisted(() => ({
  delete: vi.fn(),
  post: vi.fn(),
  put: vi.fn(),
}))

vi.mock('@/utils/request', () => ({ adminRequest }))
...
describe('管理员头像文件 API', () => {
  ...
  it('上传头像使用 FormData 的 file 字段和统一 /api 文件路径', async () => {
    ...
    adminRequest.put.mockResolvedValue({ file_url: '/files/avatar.webp' })

    await expect(uploadAdminAvatar(draft)).resolves.toEqual({
      file_url: '/files/avatar.webp',
    })

    expect(adminRequest.put).toHaveBeenCalledTimes(1)
    const [url, formData] = adminRequest.put.mock.calls[0] as [string, FormData]
  ...
  it('移除头像使用统一 /api 文件路径且不带请求体', async () => {
    ...
    adminRequest.delete.mockResolvedValue(undefined)

    await expect(removeAdminAvatar()).resolves.toBeUndefined()

    expect(adminRequest.delete).toHaveBeenCalledWith('/admin/files/profile/avatar')
  })
})
...
describe('文章图片文件 API', () => {
  ...
  it('正文图片上传使用 FastAPI 的 FormData file 字段', async () => {
    ...
    const draft = createDraft()
    adminRequest.post.mockResolvedValue({ file_url: '/files/content.webp' })

    await expect(uploadContentImage(draft)).resolves.toEqual({
      file_url: '/files/content.webp',
    })

    const [url, formData] = adminRequest.post.mock.calls[0] as [string, FormData]
  ...
  it('封面上传和移除只使用真实文章 ID', async () => {
    ...
    const draft = createDraft()
    adminRequest.put.mockResolvedValue({ file_url: '/files/cover.webp' })
    adminRequest.delete.mockResolvedValue(undefined)

    await uploadArticleCover(23, draft)
    await removeArticleCover(23)

    expect(adminRequest.put.mock.calls[0]?.[0]).toBe(
      '/admin/files/articles/23/cover',
    )
    expect(adminRequest.put.mock.calls[0]?.[1]).toBeInstanceOf(FormData)
    expect(adminRequest.delete).toHaveBeenCalledWith(
      '/admin/files/articles/23/cover',
    )
  })
  ...
  it('正文图片清理使用 JSON file_url 并保留 FastAPI 结果枚举', async () => {
    ...
    adminRequest.delete.mockResolvedValue({ result: 'retained_in_use' })
...
describe('站点背景文件 API', () => {
  ...
  it('上传背景只调用 FastAPI 的 FormData file 接口', async () => {
    ...
    const draft = createDraft()
    adminRequest.put.mockResolvedValue({ file_url: '/files/site/background.webp' })
